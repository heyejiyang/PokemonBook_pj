...
public class FileUploadService {
    ...
    private final FileInfoSaveService saveService; //파일 정보 저장
    private final FileInfoService infoService;

    /**
     * 파일 업로드 절차
     * 1. 파일 정보 저장 - gid, location : 사용자가 전달한 양식 데이터
    ...
    public List<FileInfo> uploads() {
        ...
        String gid = null, location = null; //초기화
        try {
            ...
            /* 파일 업로드 처리 S */
            for(DiskFileItem item : items){
                if(item.isFormField()){ // 일반 양식 데이터는 건너뛰기
                    continue;
                }

                //파일 정보 저장 - 파일이름=파일명.파일명.확장자 -> 마지막에 있는 . => 확장자 구분기준
                ...
                if(fileInfo == null){ // 파일 정보 저장에 실패한 경우
                    continue;
                }
