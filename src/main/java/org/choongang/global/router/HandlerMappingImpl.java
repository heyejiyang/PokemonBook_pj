...
public class HandlerMappingImpl implements HandlerMapping{
    ...
    public List<Object> search(HttpServletRequest request) {
        ...
        for (Object item : items) {
             ...
             */
            for (Method m : item.getClass().getDeclaredMethods()) {
                if (isMatch(item, request, m.getDeclaredAnnotations(), true, null)) {
                    return List.of(item, m);
                }
            }
            /* Method 애노테이션에서 체크 E */
     ...
     * @return
     */
    private boolean isMatch(Object item, HttpServletRequest request, Annotation[] annotations, boolean isMethod, String prefixUrl) {
        ...
        for (Annotation anno : annotations) {
            ...
            if (mappings != null && mappings.length > 0) {
                ...
                if (isMethod) {
                    ...
                    if (addUrl.isBlank()) {
                        for (Annotation a : item.getClass().getDeclaredAnnotations()) {
                            String[] mappingUrls2 = getMappingUrls(a, method);
                            if (mappingUrls2 != null && mappingUrls2.length > 0) {
                                addUrls.add(mappingUrls2);
                            }
                        }
                    } // endif
                    ...
                    for(String mapping : mappings) {
                        ...
                        if (addUrls.isEmpty()) {
                            ...
                            for (String[] _addurls : addUrls) {
                                for (String add : _addurls) {
                                    Pattern p = Pattern.compile("^" + request.getContextPath() + add + pattern + "$");
    ...
    private String[] getMappingUrls(Annotation anno, String method) {
