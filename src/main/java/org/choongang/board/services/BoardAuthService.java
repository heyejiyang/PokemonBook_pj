    ...
    * */
    public void check(String bId, long seq, String mode){
        ...
        if(mode.equals("write") && !memberUtil.isLogin() && authority == Authority.USER){ //회원 전용 게시판
            throw new AlertRedirectException("회원 전용 게시판입니다.", redirectUrl, HttpServletResponse.SC_UNAUTHORIZED); //메세지 뜨고 로그인 페이지로 이동
        }

        if(mode.equals("write") && !memberUtil.isAdmin() && authority == Authority.ADMIN){
            //관리자 전용 게시판
